...
"class^":function(a,b){b.el.classList.toggle(a)},classtoggle:function(a,b){b.el.classList.toggle(a)},text:v(function(a){return a.el.innerText=a.input},function(a,b){return b.el.innerText=a}),html:v(function(a){return a.el.innerHTML=a.input},function(a,b){return b.el.innerHTML=a}),attr:v(function(a,b){let c=b.el[a];return"undefined"!=typeof c?c:b.el.getAttribute(a)},function(a,b,c){c.el[a]=b;c.el.setAttribute(a,b);return b}),log:function(){var a=arguments;var b=a.length;var c=[].slice.call(a,0,b-1);
a=a[b-1];a.input&&c.push(a.input);console.log.apply(console,c)}},Y=console.error?console.error.bind(console,"TwinSpark error:"):console.log.bind(console,"TwinSpark error:"),M=Object.assign||function(a,b){if(!b)return a;for(var c in b)b.hasOwnProperty(c)&&(a[c]=b[c]);return a},X=d.requestIdleCallback||function(a){setTimeout(a,100)},Ja=d.indexedDB||d.mozIndexedDB||d.webkitIndexedDB||d.msIndexedDB,ca,ea=function(){function a(h,k){k.cb("node-remove",h);if(1!=h.nodeType)h.remove();else{h.classList.add(db);
k=h.getAnimations().filter(p=>p instanceof d.CSSTransition);if(!k.length)return h.remove();Promise.all(k.map(p=>p.finished)).then(()=>{h.remove()})}}function b(h,k){k.cb("node-insert",h);h instanceof HTMLElement&&(O(h),va(h))}function c(h,k){if(1!=h.nodeType)h.nodeValue=k.nodeValue;else{for(var p of k.attributes)I(h,k,p.name);for(p of h.attributes)I(h,k,p.name);"INPUT"==h.tagName&&"file"!=h.type?(I(h,k,"value",!0),I(h,k,"checked",!0),I(h,k,"disabled",!0)):"OPTION"==h.tagName?I(h,k,"selected"):"TEXTAREA"==
h.tagName&&(I(h,k,"value"),h.firstChild&&h.firstChild.nodeValue!=k.value&&(h.firstChild.nodeValue=k.value))}}function e(h,k){const p=new Set;for(const x of k)h.has(x)&&p.add(x);return p}function f(h,k){return h&&k&&h.nodeType==k.nodeType&&h.tagName==k.tagName}function l(h,k){if(1!=k.nodeType)return null;k=D(k,"[id]").map(p=>"#"+p.id).join(",");if(!k)return null;do if(G(h,k))return h;while(h=h.nextElementSibling);return null}function n(h,k,p){var x=k.nextSibling,J=0;do{var y=D(h,"[id]").map(K=>K.id);
if(y.length&&e(p,y).size)break;if(!y.length&&!D(k,"[id]").length&&f(h,k))return h;if(f(h,x)&&(J++,x=x.nextSibling,2<=J))break}while(h=h.nextSibling);return null}function u(h,k,p){for(;h!==k;){var x=h;h=h.nextSibling;a(x,p)}return k}function w(h,k,p){if(!p.ignoreActive||h!=g.activeElement||"INPUT"!=h.tagName&&"TEXTAREA"!=h.tagName){if(k){if(f(h,k))return c(h,k),1==h.nodeType&&1==k.nodeType&&m(h,k,p),h;h.parentElement?(h.parentElement.insertBefore(k,h.nextSibling),a(h,p),b(k,p)):h.replaceWith(k);return k}a(h,
p);return null}return h}function m(h,k,p){var x=h.firstChild,J=k.firstChild;for(k=new Set(D(k,"[id]").map(eb=>eb.id));J;){var y=J;J=y.nextSibling;if(x){var K=l(x,y);K?(x=u(x,K,p).nextSibling,w(K,y,p)):(K=n(x,y,k))?(x=u(x,K,p).nextSibling,w(K,y,p)):(h.insertBefore(y,x),b(y,p))}else h.appendChild(y),b(y,p)}x&&(x=u(x,h.lastChild,p),a(x,p))}function r(h,k,p={}){p.cb||(p.cb=function(){});h&&h.nodeType==Node.DOCUMENT_NODE&&(h=h.documentElement);k&&k.nodeType==Node.DOCUMENT_NODE&&(k=k.documentElement);return w(h,
k,p)}r.syncAttrs=c;return r}(),Q={reqs:[],request:null};N("[ts-req]",function(a){function b(c){fa([ha(a,c,!1)])}a.hasAttribute("ts-trigger")?A(a,"ts-trigger",b):ia(a,b)});N("[ts-req-batch]",function(a){function b(c){c=ha(a,c,!0);Q.reqs.push(c);Q.request||(Q.request=setTimeout(Va,16))}a.hasAttribute("ts-trigger")?A(a,"ts-trigger",b):ia(a,b)});N("[ts-action]",function(a){var b=function(c){c.detail&&c.detail.event&&(c=c.detail.event);W(H(a),c,a.getAttribute("ts-action"),null)};a.hasAttribute("ts-trigger")?
A(a,"ts-trigger",b):"A"==a.tagName||"BUTTON"==a.tagName?ia(a,b):Y("No trigger for action on element",a)});var bb=S(function(){return za({on:"visible",off:"invisible",rootMargin:"0px",threshold:.01})}),cb=S(function(){return za({on:"closeby",off:"away",rootMargin:(d.innerHeight/2|0)+"px",threshold:.01})}),$a=S(function(){return new MutationObserver(function(a){for(var b=0;b<a.length;b++)for(var c=a[b],e=0;e<c.removedNodes.length;e++)z(c.removedNodes[e],"remove",{record:c})})}),ab=S(function(){return new MutationObserver(function(a){for(var b=
...
typeof a)return M(R,a);b&&(R[a]=b);return R},elcrumbs:pa,arity:v,data:qa,merge:P,target:H,query:H,trigger:z,parseAction:ka,action:W,exec:ja,makeReq:ha,executeReqs:fa,morph:ea,setERR:a=>Y=a,_internal:{DIRECTIVES:T,FUNCS:R,init:Aa}}})(window,document,"twinspark");
