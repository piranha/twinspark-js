...
g.nodeValue;else{for(var m of g.attributes)D(f,g,m.name);for(m of f.attributes)D(f,g,m.name);"INPUT"==f.tagName&&"file"!=f.type?(D(f,g,"value",!0),D(f,g,"checked",!0),D(f,g,"disabled",!0)):"OPTION"==f.tagName?D(f,g,"selected"):"TEXTAREA"==f.tagName&&(D(f,g,"value"),f.firstChild&&f.firstChild.nodeValue!=g.value&&(f.firstChild.nodeValue=g.value))}}function d(f,g){const m=new Set;for(const r of g)f.has(r)&&m.add(r);return m}function e(f,g){return f&&g&&f.nodeType==g.nodeType&&f.tagName==g.tagName}function h(f,
g){if(1!=g.nodeType)return null;g=z(g,"[id]").map(m=>"#"+m.id).join(",");if(!g)return null;do if(B(f,g))return f;while(f=f.nextElementSibling);return null}function l(f,g,m){var r=g.nextSibling,E=0;do{var v=z(f,"[id]").map(F=>F.id);if(v.length&&d(m,v).size)break;if(!v.length&&!z(g,"[id]").length&&e(f,g))return f;if(e(f,r)&&(E++,r=r.nextSibling,2<=E))break}while(f=f.nextSibling);return null}function p(f,g,m){for(;f!==g;){var r=f;f=f.nextSibling;a(r,m)}return g}function q(f,g,m){if(!m.ignoreActive||
