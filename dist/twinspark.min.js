...
{keyPath:"url"}).createIndex("time","time")};return Y=R(a)}function Z(a,b){return a.transaction(a.name,b&&b.write&&"readwrite"||"readonly").objectStore(a.name)}function Ka(a){R(Z(a).count()).then(function(b){console.debug("PAGES IN STORAGE",b);var c=b-La;if(0<c){var d=Z(a,{write:!0}).index("time").openCursor();d.onsuccess=function(e){if(e=d.result)e.delete(),0<--c&&e.continue()}}})}function pa(){var a={url:G.pathname+G.search,html:u.body.innerHTML,time:+new Date};oa().then(function(b){R(Z(b,{write:!0}).put(a)).then(function(){Ka(b)})})}
function Ma(a,b){pa();history.pushState("history",b,a);w(t,"ts-pushstate",{url:a})}function Na(a){history.replaceState(null,"",a);w(t,"ts-replacestate",{detail:a})}function qa(a){if(a.state){var b=G.pathname+G.search;oa().then(function(c){return R(c.transaction(c.name,"readonly").objectStore(c.name).get(b))}).then(function(c){console.debug("onpopstate restore",c.url,c.html.length,c.time);c&&c.html&&(u.body.innerHTML=c.html,("initial"!=a.state||W)&&L(u.body))})}}function C(a,b){if(!a)return a;null==
...
n.classList.remove("ts-active");delete A(n)["active-xhr"]}));if(q.xhr.isAborted)return!1;if(k.ok&&k.url&&k.url!=(new URL(l,G.href)).href)return k.headers["ts-history"]=k.url,va(k.url,[u.body],k.content,k);if(k.ok)return va(l,p,k.content,k);U("Something wrong with response",k.content);p.forEach(function(n){w(n,"ts-req-error",H({error:"ts-req-error"},{response:k,url:l,opts:h}))})}).catch(function(k){T(()=>p.forEach(n=>{n.removeAttribute("aria-busy");n.classList.remove("ts-active");delete A(n)["active-xhr"]}));
U("Error retrieving backend response",l,k.error||k);p.forEach(function(n){w(n,"ts-req-error",H({error:"ts-req-error"},{response:k,url:l,opts:h}))})})}}function ca(a){return Promise.all(a.map(function(b){b.opts=Ra(b);var c={req:b},d=w(b.el,"ts-req-before",c);if(d.defaultPrevented)return null;var e=b;(c=S(b.el,d,b.el.getAttribute("ts-req-before"),c))&&(e=c.then(function(l){return d.defaultPrevented?null:b}));c=b.el.getAttribute("ts-req-strategy")||"queue";var h=A(b.el)["active-xhr"];if("first"===c&&
...
a}),html:J(function(a){return a.el.innerHTML=a.input},function(a,b){return b.el.innerHTML=a}),attr:J(function(a,b){return b.el[a]},function(a,b,c){return c.el[a]=b}),log:function(){var a=arguments;var b=a.length;var c=[].slice.call(a,0,b-1);a=a[b-1];a.input&&c.push(a.input);console.log.apply(console,c)}},U=console.error?console.error.bind(console,"TwinSpark error:"):console.log.bind(console,"TwinSpark error:"),H=Object.assign||function(a,b){if(!b)return a;for(var c in b)b.hasOwnProperty(c)&&(a[c]=
b[c]);return a},T=t.requestIdleCallback||function(a){setTimeout(a,100)},Ja=t.indexedDB||t.mozIndexedDB||t.webkitIndexedDB||t.msIndexedDB,Y,ba=function(){function a(f,g){g.cb("node-remove",f);if(1!=f.nodeType)f.remove();else{f.classList.add(db);g=f.getAnimations().filter(m=>m instanceof t.CSSTransition);if(!g.length)return f.remove();Promise.all(g.map(m=>m.finished)).then(()=>{f.remove()})}}function b(f,g){g.cb("node-insert",f);f instanceof HTMLElement&&(L(f),sa(f))}function c(f,g){if(1!=f.nodeType)f.nodeValue=
g.nodeValue;else{for(var m of g.attributes)D(f,g,m.name);for(m of f.attributes)D(f,g,m.name);"INPUT"==f.tagName&&"file"!=f.type?(f.value=g.value||"",D(f,g,"value"),D(f,g,"checked"),D(f,g,"disabled")):"OPTION"==f.tagName?D(f,g,"selected"):"TEXTAREA"==f.tagName&&(D(f,g,"value"),f.firstChild&&f.firstChild.nodeValue!=g.value&&(f.firstChild.nodeValue=g.value))}}function d(f,g){const m=new Set;for(const r of g)f.has(r)&&m.add(r);return m}function e(f,g){return f&&g&&f.nodeType==g.nodeType&&f.tagName==g.tagName}
function h(f,g){if(1!=g.nodeType)return null;g=z(g,"[id]").map(m=>"#"+m.id).join(",");if(!g)return null;do if(B(f,g))return f;while(f=f.nextElementSibling);return null}function l(f,g,m){var r=g.nextSibling,E=0;do{var v=z(f,"[id]").map(F=>F.id);if(v.length&&d(m,v).size)break;if(!v.length&&!z(g,"[id]").length&&e(f,g))return f;if(e(f,r)&&(E++,r=r.nextSibling,2<=E))break}while(f=f.nextSibling);return null}function p(f,g,m){for(;f!==g;){var r=f;f=f.nextSibling;a(r,m)}return g}function q(f,g,m){if(!m.ignoreActive||
f!=u.activeElement||"INPUT"!=f.tagName&&"TEXTAREA"!=f.tagName){if(g){if(e(f,g))return c(f,g),1==f.nodeType&&1==g.nodeType&&k(f,g,m),f;f.parentElement?(f.parentElement.insertBefore(g,f.nextSibling),a(f,m),b(g,m)):f.replaceWith(g);return g}a(f,m);return null}return f}function k(f,g,m){var r=f.firstChild,E=g.firstChild;for(g=new Set(z(g,"[id]").map(eb=>eb.id));E;){var v=E;E=v.nextSibling;if(r){var F=h(r,v);F?(r=p(r,F,m).nextSibling,q(F,v,m)):(F=l(r,v,g))?(r=p(r,F,m).nextSibling,q(F,v,m)):(f.insertBefore(v,
r),b(v,m))}else f.appendChild(v),b(v,m)}r&&(r=p(r,f.lastChild,m),a(r,m))}function n(f,g,m={}){m.cb||(m.cb=function(){});f&&f.nodeType==Node.DOCUMENT_NODE&&(f=f.documentElement);g&&g.nodeType==Node.DOCUMENT_NODE&&(g=g.documentElement);return q(f,g,m)}n.syncAttrs=c;return n}(),N={reqs:[],request:null};K("[ts-req]",function(a){function b(c){ca([da(a,c,!1)])}a.hasAttribute("ts-trigger")?x(a,"ts-trigger",b):ea(a,b)});K("[ts-req-batch]",function(a){function b(c){c=da(a,c,!0);N.reqs.push(c);N.request||(N.request=
setTimeout(Va,16))}a.hasAttribute("ts-trigger")?x(a,"ts-trigger",b):ea(a,b)});K("[ts-action]",function(a){var b=function(c){c.detail&&c.detail.event&&(c=c.detail.event);S(C(a),c,a.getAttribute("ts-action"),null)};a.hasAttribute("ts-trigger")?x(a,"ts-trigger",b):"A"==a.tagName||"BUTTON"==a.tagName?ea(a,b):U("No trigger for action on element",a)});var bb=P(function(){return wa({on:"visible",off:"invisible",rootMargin:"0px",threshold:.01})}),cb=P(function(){return wa({on:"closeby",off:"away",rootMargin:(t.innerHeight/
2|0)+"px",threshold:.01})}),$a=P(function(){return new MutationObserver(function(a){for(var b=0;b<a.length;b++)for(var c=a[b],d=0;d<c.removedNodes.length;d++)w(c.removedNodes[d],"remove",{record:c})})}),ab=P(function(){return new MutationObserver(function(a){for(var b=0;b<a.length;b++){var c=a[b];"childList"==c.type&&(w(c.target,c.target.childNodes.count?"notempty":"empty",{record:c}),w(c.target,"childrenChange",{record:c}))}})});K("[ts-trigger]",function(a){var b=a.getAttribute("ts-trigger");b&&
ha(b)[0].commands.forEach(function(c){Za(a,c)})});ka(xa);let ya,za;(null==(ya=t.performance)?void 0:null==(za=ya.navigation)?void 0:za.type)==t.performance.navigation.TYPE_BACK_FORWARD&&qa({state:"initial"});t[Aa]={onload:ka,register:K,activate:L,func:function(a,b){if("object"==typeof a)return H(O,a);b&&(O[a]=b);return O},elcrumbs:ma,arity:J,data:na,merge:M,target:C,query:C,trigger:w,parseAction:ha,action:S,exec:fa,makeReq:da,executeReqs:ca,morph:ba,setERR:a=>U=a,_internal:{DIRECTIVES:Q,FUNCS:O,init:xa}}})(window,
document,"twinspark");
