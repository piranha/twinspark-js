...
{url:L.pathname+L.search,html:g.body.innerHTML,time:+new Date},b=yield ra(),c=V(b,{write:!0});yield U(c.put(a));yield Ka(b)})}function Ma(a,b){sa();history.replaceState("history",null,null);history.pushState(null,b,a);z(d,"ts-pushstate",{url:a})}function Na(a){history.replaceState("history","",a);z(d,"ts-replacestate",{url:a})}function ta(a){return $jscomp.asyncExecutePromiseGeneratorFunction(function*(){if(a.state){var b=L.pathname+L.search,c=yield ra();c=V(c);b=yield U(c.get(b));console.debug("onpopstate restore",
